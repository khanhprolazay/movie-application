...
export function RecomendationsMovie() {
    ...
    // useEffect(() => {
    //     dispatch(moviesActions.getMovieByRecommend(0, 20));
    // }, []);
    ...
    // const MovieByRecommend = useAppSelector((state) => state.MovieByRecommend.recommend.data);
    ...
    // console.log("MovieByYear", MovieByYear)

    const disablePrev = current == 0;
    const disableNext = current == (MovieByYear.length - 1) - 4;

    const handlePrev = () => {
        current !== 0 && setCurrent((cur) => cur - 1)
    };
    const handleNext = () => {
        current < MovieByYear.length - 1 && setCurrent((cur) => cur + 1)
    };
