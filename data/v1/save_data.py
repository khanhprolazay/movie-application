...
def select_movie_image(connection: mysql.connector.MySQLConnection):
  try:
    cursor = connection.cursor()
    query = "SELECT id, imdbId, banner FROM movie"
    cursor.execute(query, ())
    row = cursor.fetchall()
    cursor.close()
    return row
...
def delete_movie(movie_imdb_id: str, movie_id: int, connection: mysql.connector.MySQLConnection): 
  cursor = connection.cursor()
  try:
    cursor.execute("DELETE FROM actor_to_movie where movieId = %s", (movie_id, ))
    cursor.execute("DELETE FROM movie where id = %s", (movie_id, ))
    print(f'\n- - - - - - - - - - DELETE SUCCESSFULLY MOVIE {movie_imdb_id.upper()} - - - - - - - - - -')
    connection.commit()
